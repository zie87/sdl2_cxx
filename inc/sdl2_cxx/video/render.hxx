...
namespace sdl2
{
  ...
  template <>
  struct enable_bitmask_operators<renderer_flags>
  {
  ...
  namespace detail
  {
    ...
    struct renderer_api
    {
      ...
      inline void get_draw_color(uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) const
      {
        SDL2_CXX_CHECK((SDL_GetRenderDrawColor(to_sdl_type(*this), &r, &g, &b, &a) >= 0));
      }
      ...
      inline void entire_copy(texture& t, const rect& dst)
      {
        SDL2_CXX_CHECK(SDL_RenderCopy(to_sdl_type(*this), to_sdl_type(t), nullptr, to_sdl_type(dst)) >= 0);
      }

      inline void entire_copy(texture& t, const double angle, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), nullptr, nullptr, angle, nullptr, static_cast<SDL_RendererFlip>(flip)) >= 0);
      }
      inline void entire_copy(texture& t, const double angle, const point& p, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), nullptr, nullptr, angle, to_sdl_type(p), static_cast<SDL_RendererFlip>(flip)) >= 0);
      }
      inline void entire_copy(texture& t, const rect& dst, const double angle, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), nullptr, to_sdl_type(dst), angle, nullptr, static_cast<SDL_RendererFlip>(flip)) >=
                       0);
      }
      inline void entire_copy(texture& t, const rect& dst, const double angle, const point& p, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(
            SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), nullptr, to_sdl_type(dst), angle, to_sdl_type(p), static_cast<SDL_RendererFlip>(flip)) >= 0);
      }
      ...
      inline void copy(texture& t, const rect& src, const rect& dst)
      {
        SDL2_CXX_CHECK(SDL_RenderCopy(to_sdl_type(*this), to_sdl_type(t), to_sdl_type(src), to_sdl_type(dst)) >= 0);
      }

      inline void copy(texture& t, const rect& src, const double angle, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), to_sdl_type(src), nullptr, angle, nullptr, static_cast<SDL_RendererFlip>(flip)) >=
                       0);
      }
      inline void copy(texture& t, const rect& src, const double angle, const point& p, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(
            SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), to_sdl_type(src), nullptr, angle, to_sdl_type(p), static_cast<SDL_RendererFlip>(flip)) >= 0);
      }
      inline void copy(texture& t, const rect& src, const rect& dst, const double angle, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(
            SDL_RenderCopyEx(to_sdl_type(*this), to_sdl_type(t), to_sdl_type(src), to_sdl_type(dst), angle, nullptr, static_cast<SDL_RendererFlip>(flip)) >= 0);
      }
      inline void copy(texture& t, const rect& src, const rect& dst, const double angle, const point& p, renderer_flip flip = renderer_flip::none)
      {
        SDL2_CXX_CHECK(
            SDL_RenderCopyEx(
                to_sdl_type(*this), to_sdl_type(t), to_sdl_type(src), to_sdl_type(dst), angle, to_sdl_type(p), static_cast<SDL_RendererFlip>(flip)) >= 0);
      }
  ...
  public:
    ...
    explicit renderer(const window& win, int idx, renderer_flags flags = renderer_flags::none)
        : detail::renderer_api<renderer>(), detail::noncopyable(),
          m_renderer(SDL_CreateRenderer(to_sdl_type(win), idx, static_cast<std::underlying_type_t<renderer_flags>>(flags)))
    {
    ...
    {
      m_renderer = std::move(w.m_renderer);
      return *this;
    }
  ...
  public:
    explicit texture(const renderer& rend, const surface& s)
        : detail::texture_api<texture>(), detail::noncopyable(), m_texture(SDL_CreateTextureFromSurface(to_sdl_type(rend), to_sdl_type(s)))
    {
      SDL2_CXX_CHECK(m_texture != nullptr);
    }

    texture(texture&& w) : m_texture(std::move(w.m_texture)) {}
    texture& operator=(texture&& w)
    {
      m_texture = std::move(w.m_texture);
      return *this;
    }
