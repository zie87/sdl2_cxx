...
namespace sdl2
{
  ...
  public:
    ...
    {
      rect out;
      if (SDL_FALSE == SDL_IntersectRect(&m_rect, to_sdl_type(r), to_sdl_type(out)))
      {
        out.set(0, 0, 0, 0);
      }
      return out;
    ...
    inline bool intersection_line(int& x1, int& y1, int& x2, int& y2) const noexcept
    {
      return SDL_TRUE == ::SDL_IntersectRectAndLine(&m_rect, &x1, &y1, &x2, &y2);
    }
    inline bool intersection_line(point& start, point& end) const noexcept
    {
      return SDL_TRUE == ::SDL_IntersectRectAndLine(&m_rect, &(start.x()), &(start.y()), &(end.x()), &(end.y()));
    }
...
inline bool operator==(const sdl2::rect& lhs, const sdl2::rect& rhs) noexcept
{
  return SDL_TRUE == ::SDL_RectEquals(sdl2::to_sdl_type(lhs), sdl2::to_sdl_type(rhs));
}
inline bool operator!=(const sdl2::rect& lhs, const sdl2::rect& rhs) noexcept
{
  return !(lhs == rhs);
