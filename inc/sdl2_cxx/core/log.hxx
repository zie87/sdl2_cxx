...
namespace sdl2
{
  struct log final
  {
    ...
    static inline void category_priority(category category, priority priority) noexcept
    {
      SDL_LogSetPriority(underlying_cast(category), static_cast<SDL_LogPriority>(priority));
    }
    ...
    {
      SDL_LogVerbose(underlying_cast(category), fmt, std::forward<Args>(args)...);
    }

    template <typename... Args>
    static void debug(category category, const char* fmt, Args&&... args) noexcept
    {
      SDL_LogDebug(underlying_cast(category), fmt, std::forward<Args>(args)...);
    }

    template <typename... Args>
    static void info(category category, const char* fmt, Args&&... args) noexcept
    {
      SDL_LogInfo(underlying_cast(category), fmt, std::forward<Args>(args)...);
    }

    template <typename... Args>
    static void warn(category category, const char* fmt, Args&&... args) noexcept
    {
      SDL_LogWarn(underlying_cast(category), fmt, std::forward<Args>(args)...);
    }

    template <typename... Args>
    static void error(category category, const char* fmt, Args&&... args) noexcept
    {
      SDL_LogError(underlying_cast(category), fmt, std::forward<Args>(args)...);
    }

    template <typename... Args>
    static void critical(category category, const char* fmt, Args&&... args) noexcept
    {
      SDL_LogCritical(underlying_cast(category), fmt, std::forward<Args>(args)...);
    }

    template <typename... Args>
    static void message(category category, priority priority, const char* fmt, Args&&... args) noexcept
    {
      SDL_LogMessage(underlying_cast(category), static_cast<SDL_LogPriority>(priority), fmt, std::forward<Args>(args)...);
    }
